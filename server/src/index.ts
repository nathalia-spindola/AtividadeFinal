...
app.post("/api/user/", (req, res) => {
  ...
  const sql = "INSERT INTO user (name, email, password, createdAt) VALUES (?,?,?,?)"
  const params = [name, email, password, createdAt]
...
app.post("/api/login/", async (req, res) => {
  ...
  database.get(sql, [email, password], (err, row) => {
    ...
    if (!row?.id) {
      res.status(401).json({ message: "user or password don't match!" })
      return
    }
